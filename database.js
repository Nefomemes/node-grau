...
(async function () {
    ...
    MongoClient.connect(url, function (err, dbClient) {
        ...
        function updateDoc(collection, docID, status) {
            return new Promise(async (resolve, reject) => {
                try {
                    (async function () {
                        if(await !collection || collection.constructor !== String)reject("Not a valid collection!");
                        if (await !status || status.constructor !== Object) reject("Not an object!");
                        if (await !docID || Number.isNaN(parseInt(docID)) || docID.constructor !== String) reject("Not a snowflake!");
                        await getDoc(docID);
                        await db.collection(collection).updateOne({ docID: docID }, {status, $currentDate: { lastModified: true }})
