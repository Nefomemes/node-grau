...
mongodb.Collection.prototype.updateDoc = function updateDoc(query, operation) {
  ...
  return new Promise((resolve, reject) => {
    try {
      (async function() {
        if (!query || !operation)
          reject("One or more parameters are missing.");
...
mongodb.Collection.prototype.giveItem =
    function giveItem(query, string, item, economyOptions) {
  const collection = this;
  return new Promise((resolve, reject) => {
    try {
      (async function() {
        if (!query || !string || !item)
          reject("One or some required parameters are missing.");
        const doc = await collection.getDoc(query);
        const ar = doc[string];
        economyOptions = {};
        var strEconomy = economyOptions.property, money = economyOptions.money;
        if (ar && ar.constructor !== Array)
          reject("The inventory string is not an array!");
